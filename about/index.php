...
<html>
<head>
<title><?=$texts[$selected_lang]['about_intro'][0]['title']?></title>
...
<div data-role="content" id="content">
	...
	<h3><?=$texts[$selected_lang]['about_intro'][0]['title']?></h3>
	<p><?=$texts[$selected_lang]['about_intro'][0]['content']?></p>

	<div data-role="collapsible-set" data-inset="false">
	
<? 
$j = 0;
foreach ($texts[$selected_lang]['about_screen'] as $row): 
	echo '		<div data-role="collapsible" data-collapsed="'.((0==$j)?'false':'true').'" data-theme="tia">'."\n";
	$j++;
?>

			<h3 class="title"><?=$row['short_title']?></h3>
<? if (!empty($row['image'])): ?>
			<img width="40%" align="left" src="<?=$docroot.$row['image']?>" />
<? endif ?>
			<h4 class="subtitle"><?=$row['title']?></h4>
			<p><?=nl2br_nolb($row['content'])?></p>
			<br clear="both" />
...
<? if ($server_is_mobile): ?>	
	<a class="button screen_no_render" href="<?=(($agent_is_mobile)?$docroot:$mdocroot)?>about/?lang=<?=$selected_lang?>&m=<?=(($agent_is_mobile)?'false':'true')?>" data-icon="refresh" rel="external"><?=$common[$selected_lang]['reload_standard']['title']?></a>
<? else: ?>
	<a class="button screen_no_render" href="<?=(($agent_is_mobile)?$docroot:$mdocroot)?>about/?lang=<?=$selected_lang?>&m=<?=(($agent_is_mobile)?'false':'true')?>" data-icon="refresh" rel="external"><?=$common[$selected_lang]['reload_mobile']['title']?></a>
<? endif ?>	
